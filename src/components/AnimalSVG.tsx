 ...
 * AnimalSVG.tsx
 * =============================================================================
 * Main orchestrator component. Renders a selectable animal in one of six poses
 * by composing the per-animal path modules and injecting pose-specific CSS
 * keyframe blocks and inline transform styles.
 *
 * === POSE CYCLE ===
 *  animating — curling animation (loops)
 *  standing  — neutral upright
 *  curled    — sleeping / resting (static)
 *  alert     — tucked but watchful (scanning animation)
 *  walking   — walk / fly / slither (loops)
 *  warning   — threat display: hackles/puff/arch/coil+rattle (loops)
 *
 * === WARNING POSE STRATEGY ===
 * Each animal path module exports a *Warning overlay component that is
 * rendered as an additive layer on top of the normal body parts. AnimalSVG
 * applies pose-specific transforms to the parent groups (body arch, tail
 * position, etc.) and injects a CSS keyframe block for the warning animations.
 * The Snake component handles its own warning mode internally (tight coil +
 * rattle geometry + head sway animation).
 * =============================================================================
 */

import { useState } from "react";
import CatBody, {
  CatLegs, CatFrontRightLeg, CatBackLeftLeg, CatBackRightLeg,
  CatTail, CatHead, CatEyes, CatFace, CatWarning,
} from "./animals/CatPaths";
import DogBody, {
  DogFrontLeftLeg, DogFrontRightLeg, DogBackLeftLeg, DogBackRightLeg,
  DogTail, DogHead, DogEyes, DogFace, DogWarning,
} from "./animals/DogPaths";
import RaccoonBody, {
  RaccoonFrontLeftLeg, RaccoonFrontRightLeg, RaccoonBackLeftLeg, RaccoonBackRightLeg,
  RaccoonTail, RaccoonHead, RaccoonEyes, RaccoonFace, RaccoonWarning,
} from "./animals/RaccoonPaths";
import ParrotBody, {
  ParrotFrontLeftLeg, ParrotFrontRightLeg, ParrotBackLeftLeg, ParrotBackRightLeg,
  ParrotTail, ParrotHead, ParrotEyes, ParrotFace,
  ParrotPerch, ParrotWingLeft, ParrotWingRight, ParrotWarning,
} from "./animals/ParrotPaths";
...
const animalParts: Partial<Record<AnimalType, {
  ...
  cat: {
    Body: CatBody, FrontLeftLeg: CatLegs, FrontRightLeg: CatFrontRightLeg,
    BackLeftLeg: CatBackLeftLeg, BackRightLeg: CatBackRightLeg,
    Tail: CatTail, Head: CatHead, Eyes: CatEyes, Face: CatFace, Warning: CatWarning,
  },
  dog: {
    Body: DogBody, FrontLeftLeg: DogFrontLeftLeg, FrontRightLeg: DogFrontRightLeg,
    BackLeftLeg: DogBackLeftLeg, BackRightLeg: DogBackRightLeg,
    Tail: DogTail, Head: DogHead, Eyes: DogEyes, Face: DogFace, Warning: DogWarning,
  },
  raccoon: {
    Body: RaccoonBody, FrontLeftLeg: RaccoonFrontLeftLeg, FrontRightLeg: RaccoonFrontRightLeg,
    BackLeftLeg: RaccoonBackLeftLeg, BackRightLeg: RaccoonBackRightLeg,
    Tail: RaccoonTail, Head: RaccoonHead, Eyes: RaccoonEyes, Face: RaccoonFace, Warning: RaccoonWarning,
  },
  parrot: {
    Body: ParrotBody, FrontLeftLeg: ParrotFrontLeftLeg, FrontRightLeg: ParrotFrontRightLeg,
    BackLeftLeg: ParrotBackLeftLeg, BackRightLeg: ParrotBackRightLeg,
    Tail: ParrotTail, Head: ParrotHead, Eyes: ParrotEyes, Face: ParrotFace, Warning: ParrotWarning,
    Extra: ParrotPerch,
  },
};
...
const AnimalSVG = ({
  ...
  const poseLabels: Record<Pose, string> = {
    ...
    warning:   animal === "snake"  ? "Rattling"  : "Warning",
  };
  ...
  // === SHARED POSE TRANSFORMS (quadrupeds + parrot) ==========================
  ...
  const headStyle: React.CSSProperties | undefined = isCurled
    ...
    : isWarning
    ? { transform: "translate(-5px, -8px) rotate(-10deg)", transformOrigin: "140px 140px", transition: "transform 0.5s ease" }
    : undefined;
  ...
  const bodyStyle: React.CSSProperties | undefined = isTucked
    ...
    // Warning: arch the back — rotate body up and slightly compress
    : isWarning
    ? { transform: "rotate(-18deg) scaleY(1.08)", transformOrigin: "250px 185px", transition: "transform 0.5s ease" }
    : undefined;
  ...
  const tailStyle: React.CSSProperties | undefined = isTucked
    ...
    // Warning: tail up and bristled / rigid
    : isWarning
    ? { transform: "rotate(-50deg) translate(5px, -10px)", transformOrigin: "340px 180px", transition: "transform 0.5s ease" }
    : undefined;
  ...
  const eyesStyle: React.CSSProperties | undefined = isCurled
    ...
    : isWarning
    ? { transform: "scaleY(1.2)", transformOrigin: "145px 117px", transition: "transform 0.4s ease" }
    : (isStanding || isWalking)
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      ...
      >
        ...
        {/* === WARNING ANIMATION (quadrupeds + raccoon) ========================
         * Body arch holds via inline bodyStyle transform.
         * Warning overlay twitches: tail bristle flick + head micro-bob.
         * ================================================================== */}
        {isWarning && animal !== "parrot" && animal !== "snake" && (
          <style>{`
            .warn-head { animation: warnHeadBob  0.5s ease-in-out infinite alternate; transform-origin: 140px 130px; }
            .warn-tail { animation: warnTailFlick 0.3s ease-in-out infinite alternate; transform-origin: 340px 140px; }
        ...
        {/* === WARNING ANIMATION (parrot) =====================================
         * Body puffs further, crest quills fan, wings drop.
         * ================================================================== */}
        {isWarning && animal === "parrot" && (
          <style>{`
            .warn-p-body { animation: warnPBodyPulse 0.6s ease-in-out infinite alternate; transform-origin: 258px 190px; }
            .warn-p-head { animation: warnPHeadBob   0.5s ease-in-out infinite alternate; transform-origin: 220px 130px; }
        ...
        {/* ================================================================== */}
        {/* SNAKE — fully self-contained renderer, handles all modes           */}
        {/* ================================================================== */}
        {animal === "snake" ? (
          <Snake
            mode={
            ...
            <g
              ...
              style={
                isCurled
                  ...
                  : isWarning
                  ? { transform: "rotate(0deg) scale(1.14)",  transformOrigin: "258px 190px", transition: "transform 0.5s ease" }
                  : undefined
              }
            >
              <parts.Body />

              {isWalking && <><ParrotWingLeft /><ParrotWingRight /></>}
              ...
              <g
                ...
                style={
                  isCurled
                    ...
                    : isWarning
                    ? { transform: "rotate(15deg)",  transformOrigin: "320px 225px", transition: "transform 0.5s ease" }
                    : isStanding
              ...
              <g
                ...
                style={
                  isCurled
                    ...
                    : isWarning
                    ? { transform: "translate(0,0) rotate(-5deg)",         transformOrigin: "220px 140px", transition: "transform 0.5s ease" }
                    : undefined
                }
              >
                <parts.Head />
                <g
                  ...
                  style={
                    isCurled
                      ...
                      : (isAlert || isWarning)
                      ? { transform: "scaleY(1.2)",  transformOrigin: "218px 108px", transition: "transform 0.4s ease" }
                      : isStanding
              ...
              {/* Warning overlay — crest + bristle feathers + open beak */}
              {isWarning && <parts.Warning />}
            </g>
          </g>

        /* ================================================================== */
        /* QUADRUPEDS (cat, dog, raccoon)                                      */
        /* ================================================================== */
        ) : parts ? (
          <g
          ...
          >
            ...
            {/* Warning overlay — hackles / arch / snarl drawn on top */}
            {isWarning && <parts.Warning />}
          </g>
        ) : null}
      </svg>
